...
The StreamOrderFilter class can be used to build up a list of options to be used in the order stream.
...
```csharp
var orderFilter = new StreamOrderFilter().WithStrategyRefs("myRef", "otherRef").WithOrdersPerStrategy();
```

## Aggregated Positions (Default)
Returns changes to the overall order position aggregated per runner.
This is the default if the StreamOrderFilter is not used or if Detailed Positions are not asked for.
```csharp
var orderFilter = new StreamOrderFilter().WithAggregatedPositions();
```
...
```csharp
var orderFilter = new StreamOrderFilter().WithOrdersPerStrategy();
```
...
## Class
You can view the StreamOrderFilter class [here](/src/Betfair/Stream/Messages/StreamOrderFilter.cs)
