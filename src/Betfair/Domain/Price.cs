...
{
    private static readonly ConcurrentDictionary<decimal, Price> _cache = new ();
    ...
    private readonly int _intOdds;

    private Price(decimal decimalOdds)
    {
        DecimalOdds = decimalOdds;
        _intOdds = (int)(decimalOdds * 100);
    }
    ...
    public Size MinimumStake =>
        Size.Of(Math.Min(Math.Ceiling(10 / DecimalOdds * 100) / 100, 2));

    public static bool operator >([NotNull]Price a, [NotNull]Price b) =>
        a._intOdds > b._intOdds;
    ...
    {
        if (_cache.ContainsKey(decimalOdds))
            return _cache[decimalOdds];
        if (!_validPrices.Contains(decimalOdds))
            decimalOdds = 1000;

        var newPrice = new Price(decimalOdds);
        _cache.TryAdd(decimalOdds, newPrice);
        return newPrice;
    }
    ...
    {
        if (_intOdds == other._intOdds) return 0;
        if (_intOdds > other._intOdds) return 1;
