...
public class ProfitTargetOrderTests
{
    [Theory]
    ...
    {
        var order = new ProfitTargetOrder(selectionId, Side.Back, 1.01, 9.99);
    ...
    {
        var order = new ProfitTargetOrder(1, Side.Back, 1.01, 9.99);
    ...
    {
        var order = new ProfitTargetOrder(1, Side.Lay, 1.01, 9.99);
    ...
    {
        var order = new ProfitTargetOrder(1, Side.Back, price, 9.99);
    ...
    {
        var order = new ProfitTargetOrder(1, Side.Back, 1.01, 9.99, PersistenceType.Persist);
    ...
    {
        var order = new ProfitTargetOrder(1, Side.Back, 1.01, 9.99, PersistenceType.Market_On_Close);
    ...
    [Fact]
    public void BetTargetTypeShouldBeBackerProfit()
    {
        var order = new ProfitTargetOrder(1, Side.Back, 1.01, 9.99);

        order.ToInstruction().LimitOrder?.BetTargetType.Should().Be("BACKERS_PROFIT");
    }
    ...
    [InlineData(1234.56)]
    [InlineData(0.01)]
    public void ProfitTargetCanBeSet(double size)
    {
        var order = new ProfitTargetOrder(1, Side.Back, 1.01, size);

        order.ProfitTarget.Should().Be(size);
